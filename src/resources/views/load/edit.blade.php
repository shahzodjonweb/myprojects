...
@section('content1')
<form  action="{{ route('load.update',$load->id) }}" enctype="multipart/form-data" method="post"  id="submitload">
    @method("PUT")
  @csrf
<div class="container ui-container">
  <h4 class="text-center font-weight-bold">Editing Load</h4>
  <hr>
  @if ($errors->any())
...
@endif
  @php
      ...
      $count=$shippers->count();
  @endphp

<div class="row mt-3">
  <div class="form-group col">
    <input type="text" class="form-control" id="number" name="number" placeholder="Load Number" value="{{ $load->number }}">
  </div>
  <div class="form-group col row">
    ...
    <div class="input-group col-8">
      ...
      <div class="row mt-3">
        <input type="hidden" name="id1" id="id1" value="{{ $shippers[0]->id }}">
        <div class="form-group col row">
          <label for="pickupDate" class="col-form-label col-3 font-weight-bold">Pickup Date</label>
          <div class="col-sm-9">
            <input type="datetime-local" class="form-control " id="pickupDate" name="pickupDate"  value="{{ $shippers[0]->localDate() }}">
          </div>
        ...
        <div class="form-group col">
          <div class="ui search " >
            <div class="ui icon input">
              <input class="prompt" type="text" placeholder="Search countries..." id="pickupLocation" name="pickupLocation" value="{{ $shippers[0]->getLocationFormat() }}">
            </div>
      ...
      @if ($count>2)
        ...
        <div class="pr{{ $i }} row mt-3">
          <input type="hidden" name="id{{ $i+1 }}" id="id{{ $i+1 }}" value="{{ $shippers[$i]->id }}">
          <div class="form-group col row">
            <label for="deliveryDate" class="col-form-label col-3 font-weight-bold">Location {{ $i }}</label>
            <div class="col-sm-9">
              <input type="datetime-local" class="form-control " id="date{{ $i }}" name="date{{ $i }}"  value="{{ $shippers[$i]->localDate() }}">
            </div>
          ...
          <div class="ui search " >
            <div class="ui icon input">
              <input class="prompt" type="text" placeholder="Search countries..." id="location{{ $i }}" name="location{{ $i }}" value="{{ $shippers[$i]->getLocationFormat() }}">
            </div>
            <div class="results"></div>
          </div>
        </div>
...
@endfor
    ...
    <input type="hidden" name="lastid" id="lastid" value="{{ $count-1 }}">
...
<div class="row mt-3">
  <input type="hidden" name="iddel" id="id{{ $count }}" value="{{ $shippers[$count-1]->id }}">
  <div class="form-group col row">
    <label for="deliveryDate" class="col-form-label col-3 font-weight-bold">Delivery Date</label>
    <div class="col-sm-9">
      <input type="datetime-local" class="form-control " id="deliveryDate" name="deliveryDate"  value="{{ $shippers[$count-1]->localDate() }}">
    </div>
  ...
  <div class="form-group col">
    <div class="ui search " >
      <div class="ui icon input">
        <input class="prompt" type="text" placeholder="Search countries..." id="deliveryLocation" name="deliveryLocation" value="{{ $shippers[$count-1]->getLocationFormat() }}">
      </div>
      <div class="results"></div>
    </div>
  </div>
    ...
    <div class="row col-6">
      ...
      <div class="row mt-3">
        <div class="row col">
          ...
          <div class="col-8">
            <div class="input-group">
              ...
              <input type="number" class="form-control" id="price" name="price" value="{{ $load->price }}">
              <div class="input-group-append">
        ...
        <div class="form-group col row">
          ...
          <select class="form-control font-italic col-8" id="driver" name="driver" value="{{ $load->driver_id }}">
        ...
        <div class="form-group col row">
          ...
          <select class="form-control font-weight-bold col-8" id="status" name="status" value="{{ $load->status }}">
      ...
      <div class="row mt-3">
        <div class="row col">
            <label for="detention" class="col-form-label col-3 font-weight-bold">Detention</label>
            <div class="col-8">
              <div class="input-group">
                <div class="input-group-prepend ">
                  <span class="input-group-text">$</span>
                </div>
                <input type="number" class="form-control" id="detention" name="detention" value="{{ $load->detention }}">
                <div class="input-group-append">
                  <span class="input-group-text">.00</span>
                </div>
              </div>
          ...
          <div class="row col">
            <label for="lumper" class="col-form-label col-3 font-weight-bold">Lumper</label>
            <div class="col-8">
              <div class="input-group">
                <div class="input-group-prepend ">
                  <span class="input-group-text">$</span>
                </div>
                <input type="number" class="form-control" id="lumper" name="lumper" value="{{ $load->lumper }}">
                <div class="input-group-append">
                  <span class="input-group-text">.00</span>
                </div>
              </div>
          ...
          <div class="row col">
            <label for="tonu" class="col-form-label col-3 font-weight-bold">Tonu</label>
            <div class="col-8">
              <div class="input-group">
                <div class="input-group-prepend ">
                  <span class="input-group-text">$</span>
                </div>
                <input type="number" class="form-control" id="tonu" name="tonu" value="{{ $load->tonu }}">
                <div class="input-group-append">
                  <span class="input-group-text">.00</span>
                </div>
              </div>
      ...
      <div class="row mt-3">
        <div class="form-group col">
          <textarea class="form-control" id="comment" name="comment" rows="2" placeholder="Comments here ...">{{ $load->comment }}</textarea>
        </div>
      </div>
      <hr>
      <div class="row mt-3">
        ...
        <div class="col-4 row">
            <label for="status" class="col-form-label col-3">Upload BOL</label>
            <div class="col-9">
              <div class="custom-file ">
                <input type="file" class="custom-file-input" id="bol" name="bol">
                <label class="custom-file-label" for="bol">Choose file</label>
              </div>
            </div>
          </div>
      ...
      <div class="row d-flex flex-row-reverse justify-content-center">
        <button type="button" class="btn btn-primary mr-2 btn-block btn-lg mt-5 mb-3 w-50 font-weight-bold"  onclick="checker()">Update</button>
      </div>
...
<script>
 $("#driver").val({{ $load->driver_id}}).change();
 $("#status").val("{{ $load->status}}").change();
 $("#broker").val({{  $load->broker->id}}).change();
 rc='{{  $load->rc}}';
 bol='{{  $load->bol}}';
 $("#rc").next('.custom-file-label').html('...'+rc.substr(rc.length - 15));
 $("#bol").next('.custom-file-label').html('...'+bol.substr(bol.length - 15));
</script>
<script>
  $('#tab-1').prop( "checked", false );
